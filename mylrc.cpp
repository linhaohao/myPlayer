    ...
    QLabel(parent)
{
    ...
    setText(tr("MyPlayer音乐播放器---紫菜蛋花汤作品"));
...
void MyLrc::paintEvent(QPaintEvent *)
{
    ...
    painter.drawText(1,1,800,60,Qt::AlignLeft,text().toUtf8());

    //再在上面绘制渐变文字
    painter.setPen(QPen(linearGradient,0));
    painter.drawText(0,0,800,60,Qt::AlignLeft,text().toUtf8());

    //设置歌词遮罩
    painter.setPen(QPen(maskLinearGradient,0));
    painter.drawText(0,0,lrcMaskWidth,60,Qt::AlignLeft,text().toUtf8());
}
//两个鼠标事件处理函数实现了部件的拖动
